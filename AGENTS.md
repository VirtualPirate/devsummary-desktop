...
- **Provider** (`src/common/llm/`): OpenAI, Gemini and the three agent CLIs (Claude Code, OpenCode, Cursor) are served by subclasses of one abstract `LlmClient`, which is also the DI token — callers never learn which one answered. The AI settings page writes `LLM_PROVIDER`, and the client re-resolves it on every call, so switching provider, pasting a key or installing a CLI needs no restart. Gemini is reached through its OpenAI-compatible endpoint, so those two share one SDK and one call shape. **An agent CLI is not an API key**: it spawns the `claude`, `opencode` or `agent` binary already installed and logged in on the machine, with the prompt on stdin — Claude Code takes its schema and system prompt on argv, OpenCode takes both through an env var, and Cursor takes both in stdin while `--mode ask` locks tools read-only and `--trust` enables headless execution in an empty scratch workspace — see the backend AGENTS.md for the adapter seam and detection rules.
- **Commit analysis** (`src/integrations/github/commit-analysis/`): each non-merge commit's message + diff (up to 60k chars) is classified by the configured provider (`gpt-4o-mini` / `gemini-3.1-flash-lite` / `haiku` / `opencode/big-pickle` / `composer-2.5-fast` by default, `OPENAI_COMMIT_ANALYSIS_MODEL` / `GEMINI_COMMIT_ANALYSIS_MODEL` / `CLAUDE_CODE_COMMIT_ANALYSIS_MODEL` / `OPENCODE_COMMIT_ANALYSIS_MODEL` / `CURSOR_COMMIT_ANALYSIS_MODEL`) into a structured result: `commit_type` (one of `fix`, `feature`, `optimization`, `refactor`, `docs`, `test`, `chore`), summary, and changes list. Analyses are cached per commit.
- **Brief generation** (`src/briefs/generation/`): scope label, date range, and per-commit analyses are assembled into a prompt (~30k char cap) and sent to the configured provider (`gpt-4o-mini` / `gemini-3.1-flash-lite` / `sonnet` / `opencode/big-pickle` / `composer-2.5` by default, `OPENAI_BRIEF_MODEL` / `GEMINI_BRIEF_MODEL` / `CLAUDE_CODE_BRIEF_MODEL` / `OPENCODE_BRIEF_MODEL` / `CURSOR_BRIEF_MODEL`) with structured output (Zod) returning `{ title, summary }`. The system prompt mandates jargon-free, achievement/impact-oriented writing for executives.
