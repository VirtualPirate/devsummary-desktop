...
- **Commit analysis** (`src/integrations/github/commit-analysis/`): each non-merge commit's message + diff (up to 60k chars) is classified by OpenAI (`gpt-4o-mini` by default, `OPENAI_COMMIT_ANALYSIS_MODEL` env) into a structured result: `commit_type` (one of `fix`, `feature`, `optimization`, `refactor`, `docs`, `test`, `chore`), summary, and changes list. Analyses are cached per commit.
- **Brief generation** (`src/briefs/generation/`): scope label, date range, and per-commit analyses are assembled into a prompt (~30k char cap) and sent to OpenAI (`gpt-4o-mini` by default, `OPENAI_BRIEF_MODEL` env) with structured output (Zod) returning `{ title, summary }`. The system prompt mandates jargon-free, achievement/impact-oriented writing for executives.
