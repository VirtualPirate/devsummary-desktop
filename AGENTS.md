...
- **Provider** (`src/common/llm/`): OpenAI and Gemini are two subclasses of one abstract `LlmClient`, which is also the DI token — callers never learn which one answered. The AI settings page writes `LLM_PROVIDER`, and the client re-resolves it on every call, so switching provider or pasting a key needs no restart. Gemini is reached through its OpenAI-compatible endpoint, so there is one SDK and one call shape — see the backend AGENTS.md for the details that differ.
- **Commit analysis** (`src/integrations/github/commit-analysis/`): each non-merge commit's message + diff (up to 60k chars) is classified by the configured provider (`gpt-4o-mini` / `gemini-3.1-flash-lite` by default, `OPENAI_COMMIT_ANALYSIS_MODEL` / `GEMINI_COMMIT_ANALYSIS_MODEL`) into a structured result: `commit_type` (one of `fix`, `feature`, `optimization`, `refactor`, `docs`, `test`, `chore`), summary, and changes list. Analyses are cached per commit.
- **Brief generation** (`src/briefs/generation/`): scope label, date range, and per-commit analyses are assembled into a prompt (~30k char cap) and sent to the configured provider (`gpt-4o-mini` / `gemini-3.1-flash-lite` by default, `OPENAI_BRIEF_MODEL` / `GEMINI_BRIEF_MODEL`) with structured output (Zod) returning `{ title, summary }`. The system prompt mandates jargon-free, achievement/impact-oriented writing for executives.
