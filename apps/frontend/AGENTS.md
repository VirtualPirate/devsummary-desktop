...
src/
  ...
  components/
    ...
    integrations/            # GitHub + AI credential forms and status cards
...
The viewer's zone is almost never the right zone here. See the root `AGENTS.md` "Timezones" section for the model — the rules there are the distilled findings of the timezone audit.
...
- **A date the user picked** in `<Input type="date">` is a calendar date in *their* zone. Build the instant from its parts (`new Date(y, m - 1, d)`) before sending it — `new Date("2026-08-14")` parses as UTC midnight, which is what silently dropped a day's worth of briefs from the list filter at UTC+10. `lib/day-boundary.ts`'s `localDayBoundary` is the pattern (shared by the briefs list and the commits explorer); note that it sends *both* bounds as exclusive local midnights, because the server compares them against a brief's exclusive `periodEnd`.
...
The user's GitHub PAT and AI provider key (OpenAI or Gemini — the AI page has a provider select, and each provider's key is stored separately) are pasted on their own integrations page (`routes/integrations-github.tsx`, `routes/integrations-ai.tsx`, tabbed by `components/integrations/integration-tabs.tsx`). `routes/settings.tsx` keeps only desktop, theme, data directory and workspace — **neither email nor Slack delivery is available in the desktop version** (deviation D-H, and **Deliberately not included** in the root AGENTS.md), so there is no SMTP form, no email recipient field, no Slack page and no channel picker anywhere. Neither is a keychain item. The backend hands the whole set to the Electron main process, which encrypts it with `safeStorage` into a single `userData/secrets.bin` (mode `0600`) — the OS credential store holds the key to that file, not the credentials. The bundle comes back as environment variables at `utilityProcess.fork`, and `SecretsService.update` writes each into `process.env` live so a pasted key works without a restart. Say "encrypted on this machine by your OS credential store" in UI copy, never "in your keychain". **Nothing is ever read back**: `GET /api/local-settings` answers booleans only, so a status pill is the most the UI can show. GitHub is the exception — its status comes from `useGithubInstallations()`, because the token is stored by `POST /api/integrations/github/token` (which also validates it and reconciles the repository list) rather than through the credentials endpoint. A brief is delivered as a desktop notification and read in the app, so a schedule has no delivery step and a brief detail view has no re-send button.
