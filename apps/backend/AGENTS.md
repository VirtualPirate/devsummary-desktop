...
├── KyselyModule (global) ─── PGlite + KYSELY_DB token; migrates (behind a backup) in its factory
...
- **Not every CLI is configured on argv**, which is what the adapter's optional hooks are for — `env(req)` for a child environment, `stdin(req, userPrompt)` for a CLI that wants the system prompt and schema in the prompt body (Cursor), `workspaceDir` for one that reads instruction files out of its working directory (the client creates it recursively *and* spawns the child in it), `maxConcurrent` to lower the shared five-process cap (only OpenCode does), and `files(req)` for one that takes an argument as a path rather than a value (Codex's `--output-schema`; the client creates each parent directory and writes the content, tmp-file-then-`rename`, because the paths are deterministic and two concurrent calls can name the same file). OpenCode has no `--system-prompt`, no `--json-schema` and no tools flag: its whole agent — system prompt *and* `permission {"*": "deny"}` — is inline JSON in `OPENCODE_CONFIG_CONTENT`, selected with `--agent devsummary`. `runCli` merges that over the child environment (never replacing it), and the client feeds `env()` and `buildArgs()` the same request object so they cannot describe different calls.

- **A model catalogue is offered, never enforced** (`model-catalog.ts`, `GET /api/local-settings/models`). Each adapter says how to list: `agent --list-models` and `opencode models` print one, `claude` and `codex` have no such command and carry a fixed list instead. A keyed provider is its account's `/models` listing, minus the ids that cannot answer a chat completion (embeddings, speech, images), with Gemini's `models/` prefix stripped. **Every failure answers `[]`** — no binary, no key, a non-zero exit, an unreachable API — because the picker still accepts a typed id, which is the only thing that works for a model released this morning. Nothing validates a stored model against this list. Two things that cost a debugging round each: **stdout is `stripAnsi`'d before an adapter parses it**, because `pnpm` and the Electron shell both export `FORCE_COLOR=1` and `agent --list-models` then wraps every id in SGR codes (11 151 chars plain, 15 454 coloured, 0 models parsed) — a listing looks plain in a terminal and is not one here; and each failure **logs its own reason** (no binary / `exited <code>` + first stderr line / parsed nothing from N chars), since an empty array is a legitimate answer and therefore the one shape that cannot be diagnosed from the screen.
- **The child environment is the backend's minus the credential bundle.** `childEnv` in `run-cli.ts` deletes every `SECRET_KEYS` entry — the GitHub PAT, the provider key, `DB_ENCRYPTION_KEY` — before the spawn, because `SecretsService` writes the decrypted bundle into `process.env` by design and every one of these children is a model running on an untrusted commit diff. It is done at the single spawn rather than per adapter, so claude-code, opencode, cursor, codex and the detector's `$SHELL -lic` probe are covered by one guard, and so is the next adapter. A deny-list, not an allow-list: a CLI still has to find its own login, config, cache and home. `opts.env` is merged *after* the strip, so an adapter with a real need for one of those keys states it rather than inheriting it; none does.
...
PG schema namespaces: `public` (`demo`, `organizations`, `organization_members`, `organization_invites`, `jobs`, `local_settings`), `auth` (`user`, `session`, `account`, `verification` — Better Auth's tables, now holding one seeded user row; the shipped migrations are frozen so they stay), `github` (`installations`, `repositories`, `repository_branches`, `commits`, `commit_branches`, `commit_analyses`, `collaborators`, `repository_collaborators`, `webhook_events`), `slack` (`installations`), `briefs` (`briefs`, `brief_commits`, `brief_schedules`, `projects`, `project_repositories`, `teams`, `team_collaborators`), `agents` (`sessions`, `usage`, plus the four `checkpoint*` tables `PostgresSaver.setup()` creates for itself), `marketing` (`waitlist`). Several of those tables are now unused (`organization_invites`, `webhook_events`, `slack.installations`, `marketing.waitlist`, most of `auth`) — the migrations that create them are shipped and **never edited**, so the tables stay and nothing reads them. Same for the `slack_installation_id` / `slack_channel_id` columns on `brief_schedules` and `delivery_slack_channel_id` on `briefs`: Slack was removed from the product (root AGENTS.md, **Deliberately not included**) and the columns stayed.
...
- `LOG_LEVEL` env (default `info`); redacts `authorization`, `cookie`, `x-desktop-token` and `set-cookie`. Log calls interpolate `err.message`/`err.stack`, never a raw error object — that is what keeps an Octokit `RequestError` from serializing `err.request.headers.authorization` into the file.
...
**`JobQueueService.enqueue(type, args, opts)`** — the producer. `opts.id` is a stable dedup key, which is what `startDeduped` / `workflowIdConflictPolicy: USE_EXISTING` bought — a second enqueue over a `pending` or `running` row is a no-op, and over a **terminally failed** one it re-arms the row (`state='pending'`, `attempts=0`, `error=null`, fresh `run_at`, and the caller's `args` — a checkpointed cursor belongs to the run that died). Without that re-arm a dead row holds its id forever, and every id here is derived from state rather than time: a failed `sweep:<repo>:<branch>:<runDate>` would block that repository's ingest until the next runDate, and a failed `brief:<id>` would make the stale-pending reaper a silent no-op. **The re-arm is rate-limited to once an hour** (`REARM_COOLDOWN_MS`, measured against the failed row's `run_at`): it resets `attempts`, so an uncapped one hands a deterministically failing handler a fresh retry budget on every scheduler tick — 4 attempts per 15 minutes for the sweep (~380 GitHub calls/day on the user's own PAT), 4 per reap for `brief:<id>` at an LLM call each. `opts.force` skips the cooldown and is for **user-triggered** enqueues only (`POST …/commits/backfill` and `…/commits/analyze` — the manual retry buttons); schedulers, fan-outs and reapers must never set it. Ids: `scan:<repositoryId>:<branch>`, `analyze:<repositoryId>:<branch>:<sinceISO>`, `brief:<briefId>`, `sweep:<repositoryId>:<branch>:<runDate>`, `dispatch:<YYYY-MM-DDTHH:mm>`. `opts.phase` and `opts.organizationId` are the old `Phase` / `OrganizationId` search attributes, now columns.
...
**A paging handler checkpoints its cursor.** A retry re-invokes the handler with whatever `args` the row holds, so a loop that does not write its position back starts again at page 1 — on `github.sweep` that re-enqueues every repository already ingested this run (their rows were deleted on success, so the stable id no longer dedupes them), and on `analysis.analyzeRepo` it replans every page and, under `force`, re-spends their LLM calls. `JobQueueService.saveArgs(jobId, args)` writes the resume point onto the running row; `analyzeRepo`, `sweep` and `backfillRepoLocStats` call it at each page boundary (the `job` the registry hands the handler is where `jobId` comes from), so a retry costs one page. Write the checkpoint **after** the page, never before — a crash then resumes at the page that was interrupted instead of skipping it.
...
| `api/organizations/current/briefs` | briefs/generation | list (cursor pagination + filters), get, `GET /:id/commits`, `GET /:id/report`, `POST /generate` (202 + jobId). No `POST /:id/deliver`: there is one channel and re-notifying this machine is not a retry |
...
| `api/local-settings` | local/settings | `GET` status (booleans + dataDir + effective models), `GET /usage` (token totals), `GET /agents` (installed agent CLIs, `?refresh=1` forces a re-detect), `GET /models?provider=` (model ids for the picker), `PUT /credentials`, `POST /agents/:id/test` |
...
4. **Deliver** (`delivery/`) — `BriefDelivererService` fires a desktop notification when enabled, then sets brief status `delivered` (+ `failureReason`) and `schedule.lastSentAt`. Backfilled briefs skip delivery. **There is no email channel and no Slack channel** — see deviation D-H and **Deliberately not included** in the root AGENTS.md.
...
**Slack delivery is not available either**, and was removed rather than never ported — see **Deliberately not included** in the root AGENTS.md for the full list of what went and what stayed. There is no `src/integrations/slack/`, no `BriefSlackService`, no `BriefRenderService` (Block Kit was its only consumer), no `SLACK_BOT_TOKEN` and no `@slack/web-api`. Do not add one back.

**Desktop notification** is therefore the only channel. `BriefDesktopService` posts `{ type: 'notification', title, body, briefId }` over `process.parentPort` and the Electron main process shows a native notification; `RenderableBrief` lives in that file now. It is `@Optional()` in `BriefDelivererService` and gated on a toggle in `local_settings`; outside Electron it fails with "desktop channel unavailable" rather than pretending. A successful send **is** recorded in `delivered_channels`, which is what a caller asking "did this actually go out?" reads — `status` is a whole-brief verdict. There is no manual re-send at all: `deliverOne` and `POST /briefs/:id/deliver` are gone, since re-notifying the machine the user is already looking at is not a retry.

**A delivery failure never writes `status: 'failed'`.** The brief was written and is readable; `failed` is a *generation* verdict everywhere downstream, and writing it here hid a finished summary behind a regenerate button that would re-spend the LLM call. `failureReason` is the whole record.

`BriefDeliveryChannel` is still `'email' | 'slack' | 'desktop'` in both the DTO and the database types. `'email'` and `'slack'` are **read-only history**: nothing writes either any more, but a `delivered_channels` row written before D-H / the Slack removal still carries them.
...
**Unit tests**: `*.spec.ts` under `src/` (convention: colocated `__tests__/` dirs). ESM-only packages don't work with Jest (CJS), so manual mocks in `src/__mocks__/` are wired via `moduleNameMapper` in package.json for: `@octokit/core`, `@octokit/plugin-paginate-rest`, `openai` (+ `/helpers/zod`).
...
**The job runner and the scheduler are live in every booted e2e app.** Never `sleep()` to wait for background work — poll with `waitForJobs(db)` from `test/e2e/harness/wait-for-jobs.ts`, which drains on the empty table and re-throws a failed job's own error instead of timing out silently. `pipeline.e2e.spec.ts` is the end-to-end example: PAT connect → branch pick → scan/backfill/analyze → project → brief generated and delivered as a desktop notification over the `shell` fake.
...
- `DB_ENCRYPTION_KEY` — the passphrase AES-256-GCM keys derive from, protecting the stored GitHub PAT. Generated once and then **stable**; `SecretsService` falls back to an ephemeral per-boot key with a warning rather than a hardcoded default.
...
**Gone:** `DATABASE_URL` (the agent checkpointer gets a `pg.Pool` shim over PGlite instead — see Agents), `AGENTS_ENABLED`, `BETTER_AUTH_*`, `GOOGLE_*`, `RESEND_API_KEY`, `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASS`/`EMAIL_FROM` (D-H), `GITHUB_APP_*`, `GITHUB_WEBHOOK_SECRET`, every `SLACK_*` (including `SLACK_BOT_TOKEN` — Slack was removed), `FRONTEND_URL` (the Slack "view in app" link was its only reader), `TEMPORAL_*`, `INTERNAL_API_TOKEN`.
