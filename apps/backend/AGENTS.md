...
| `LiveLlmClient` | `live-llm.client.ts` | **desktop-only**, see below — resolves settings per call and delegates to one of the three above |
...
- `LLM_PROVIDER` — `openai` (default) or `gemini`. Written by the AI page; decides which key and which model vars are read.
- `OPENAI_API_KEY` / `GEMINI_API_KEY` — the selected provider's key is the one that matters; each is shared by commit analysis and brief generation. Only one needs to be set.
- `OPENAI_COMMIT_ANALYSIS_MODEL`, `OPENAI_BRIEF_MODEL` (default `gpt-4o-mini`) and `GEMINI_COMMIT_ANALYSIS_MODEL`, `GEMINI_BRIEF_MODEL` (default `gemini-3.1-flash-lite`). Read live, so a change applies to the next job. Not in the bundle and not settable from the app: `COMMIT_ANALYSIS_LLM_PROVIDER` / `BRIEFS_LLM_PROVIDER`, honoured from env only.
