...
describe('BriefRenderService.emailSubject', () => {
  const base = {
    id: 'b1',
    title: 'Delivery Integrations, Data Integrity, and Marketing Updates',
    briefInfoTitle: 'i',
    summary: 's',
    highlights: [],
  };

  it('names the scope, never the AI title', () => {
    const svc = new BriefRenderService();
    expect(
      svc.emailSubject({
        ...base,
        scope: { type: 'project', name: 'Mobile' },
        cadence: 'weekly',
      }),
    ).toBe("Mobile project's work report is ready");
    expect(
      svc.emailSubject({
        ...base,
        scope: { type: 'collaborator', name: 'octocat' },
        cadence: 'daily',
      }),
    ).toBe("octocat's developer work report is ready");
  });
