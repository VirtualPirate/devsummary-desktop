...
describe('OrganizationsService', () => {
  ...
  describe('transferOwnership', () => {
    ...
    it('allows transfer when the target already owns another org', async () => {
      const { orgsRepo, membersRepo, db, teardown } = makeMocks();
      membersRepo.findByOrgAndUser.mockImplementation(
        async (_orgId: string, userId: string) =>
          userId === 'user-2'
      ...
      const svc = new OrganizationsService(orgsRepo, membersRepo, db, teardown);
      const out = await svc.transferOwnership({
        organizationId: 'org-1',
        currentOwnerUserId: 'user-1',
        newOwnerUserId: 'user-2',
      });
      expect(out.ownerId).toBe('user-2');
    });
    ...
    it('flips owner_id and swaps role rows in a transaction', async () => {
      ...
      // so promoting the target first would abort the transfer.
      expect(
        membersRepo.updateRole.mock.calls.map((c: unknown[]) => c[1]),
      ).toEqual(['admin', 'owner']);
    });

    it('locks the organization row before reading memberships', async () => {
      const { orgsRepo, membersRepo, db, teardown } = makeMocks();
      const order: string[] = [];
      orgsRepo.lockById.mockImplementation(async () => {
        order.push('lock');
        return { id: 'org-1' };
      });
      membersRepo.findByOrgAndUser.mockImplementation(
      ...
      expect(orgsRepo.lockById).toHaveBeenCalledWith(
        'org-1',
        expect.anything(),
      );
    ...
    it('404s when the organization vanished before the lock', async () => {
      const { orgsRepo, membersRepo, db, teardown } = makeMocks();
      orgsRepo.lockById.mockResolvedValue(null);
